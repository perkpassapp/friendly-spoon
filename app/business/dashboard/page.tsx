...
type Deal = {
  ...
  address: string; active: boolean; schedule: Schedule | null; admin_disabled: boolean; photo_url?: string
}
type Redemption = { id: string; member_email: string; deal_description: string; code: string; redeemed_at: string }
type Stats = { total: number; thisMonth: number; lastMonth: number; byDay: Record<string, number> }
...
export default function BusinessDashboard() {
  ...
  const [tab, setTab] = useState<'overview' | 'deals' | 'submit' | 'photo'>('overview')
  ...
  async function handleSetCode() {
    ...
    setLoading(true); setError('')
    await supabase.from('business_accounts').update({ access_code: newCode }).eq('id', account!.id)
    const updated = { ...account!, access_code: newCode }
    setAccount(updated); setSettingCode(false); await loadDashboard(updated)
  }
  ...
  async function loadDashboard(biz: BusinessAccount) {
    ...
    const thisMonth = r.filter((x: Redemption) => x.redeemed_at >= thisMonthStart).length
    const lastMonth = r.filter((x: Redemption) => x.redeemed_at >= lastMonthStart && x.redeemed_at < thisMonthStart).length
    const byDay: Record<string, number> = {}
    const days = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat']
    r.forEach((x: Redemption) => { const d = days[new Date(x.redeemed_at).getDay()]; byDay[d] = (byDay[d] || 0) + 1 })
  ...
  async function deleteDeal(deal: Deal) {
    ...
    setDeals(prev => prev.filter(d => d.id !== deal.id)); setConfirmDelete(null); setDeletingDeal(null)
  }

  function openScheduleEditor(deal: Deal) {
    setEditingSchedule(deal)
    if (deal.schedule) { setSchedDays(deal.schedule.days); setSchedStart(deal.schedule.start); setSchedEnd(deal.schedule.end) }
    else { setSchedDays([0,1,2,3,4,5,6]); setSchedStart('09:00'); setSchedEnd('21:00') }
  }
  ...
  async function saveSchedule() {
    ...
    setEditingSchedule(null); setSavingSchedule(false)
  }
    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          ...
          <p className="fade-up-2" style={{ fontSize: '15px', fontWeight: 500, color: 'var(--ink-3)', marginBottom: '32px' }}>First time here — set a private access code to secure your dashboard going forward.</p>
          <div className="fade-up-3">
            {[{ label: 'Access code', val: newCode, set: setNewCode, placeholder: 'Min. 4 characters' }, { label: 'Confirm code', val: confirmCode, set: setConfirmCode, placeholder: 'Repeat your code' }].map(f => (
            ...
            <button onClick={handleSetCode} disabled={loading} className="btn btn-primary" style={{ width: '100%', fontSize: '17px', padding: '16px', marginTop: '8px' }}>{loading ? 'Saving...' : 'Set code & enter dashboard'}</button>
          </div>
        </div>
      </div>
    </main>
    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          ...
          <p className="fade-up-2" style={{ fontSize: '16px', fontWeight: 500, color: 'var(--ink-3)', marginBottom: '32px' }}>Enter your email and access code.</p>
          <div className="fade-up-3">
            {[{ label: 'Email address', val: email, set: setEmail, type: 'email', placeholder: 'you@yourbusiness.com' }, { label: 'Access code', val: accessCode, set: setAccessCode, type: 'password', placeholder: 'Your private access code' }].map(f => (
              <div key={f.label} style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '6px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-3)' }}>{f.label}</label>
                <input type={f.type} value={f.val} onChange={e => f.set(e.target.value)} placeholder={f.placeholder} className="pp-input" onKeyDown={e => e.key === 'Enter' && handleLogin()} />
              </div>
            ))}
            <p style={{ fontSize: '12px', color: 'var(--ink-4)', fontWeight: 500, marginBottom: '16px' }}>First time logging in? Enter your email and any code — you will be prompted to set your access code.</p>
            {error && <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--red)', marginBottom: '12px' }}>{error}</p>}
            <button onClick={handleLogin} disabled={loading || !email} className="btn btn-primary" style={{ width: '100%', fontSize: '17px', padding: '16px', marginBottom: '16px' }}>{loading ? 'Loading...' : 'View my dashboard'}</button>
            <div style={{ borderTop: '1px solid var(--border-2)', paddingTop: '16px', textAlign: 'center' }}>
              ...
              <Link href="/for-business" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '15px', textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--green-dk)', textDecoration: 'none' }}>Apply to join PerkPass</Link>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', paddingBottom: '80px' }}>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '32px', borderBottom: '2px solid var(--ink)', overflowX: 'auto' }}>
          {([{ key: 'overview', label: 'Overview' }, { key: 'deals', label: 'My Deals (' + deals.length + ')' }, { key: 'submit', label: 'Submit Deal' }, { key: 'photo', label: 'My Photo' }] as const).map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '15px', textTransform: 'uppercase', letterSpacing: '0.04em', padding: '10px 16px', border: 'none', cursor: 'pointer', background: 'none', color: tab === t.key ? 'var(--ink)' : 'var(--ink-4)', borderBottom: tab === t.key ? '2px solid var(--ink)' : '2px solid transparent', marginBottom: '-2px', flexShrink: 0 }}>{t.label}</button>
          ))}
        </div>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '32px' }}>
              ...
              <div style={{ background: 'var(--bg-2)', borderRadius: '10px', padding: '24px', border: '1px solid var(--border-2)' }}>
                ...
                <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--ink-4)' }}>This month{monthChange !== 0 && <span style={{ marginLeft: '6px', color: monthChange > 0 ? 'var(--green-dk)' : 'var(--red)' }}>{monthChange > 0 ? '+' : ''}{monthChange}%</span>}</div>
              </div>
              <div style={{ background: 'var(--bg-2)', borderRadius: '10px', padding: '24px', border: '1px solid var(--border-2)' }}>
              ...
              <div style={{ background: 'var(--bg-2)', borderRadius: '10px', padding: '24px', border: '1px solid var(--border-2)' }}>
                <div className="display" style={{ fontSize: '52px', color: 'var(--ink)', lineHeight: 1, marginBottom: '4px' }}>{Object.entries(stats.byDay).sort((a, b) => b[1] - a[1])[0]?.[0] || '--'}</div>
            ...
            {redemptions.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {redemptions.slice(0, 20).map((r: Redemption) => (
            ...
            {account?.admin_disabled ? (
              <div style={{ background: 'var(--red-lt)', borderRadius: '10px', padding: '28px', textAlign: 'center', border: '1px solid var(--red)', marginBottom: '24px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'var(--red-lt)', border: '2px solid var(--red)', margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--red)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>
                </div>
                <div className="display" style={{ fontSize: '24px', color: 'var(--red)', marginBottom: '8px' }}>Account suspended.</div>
                <p style={{ fontSize: '14px', fontWeight: 500, color: 'var(--red)', lineHeight: 1.5 }}>Your account has been disabled by PerkPass. Your deals are no longer visible to members and cannot be managed until your account is reinstated.</p>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--red)', marginTop: '12px' }}>Please contact PerkPass to resolve this.</p>
                <a href="tel:6105337791" style={{ display: 'inline-block', marginTop: '12px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '18px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--red)', textDecoration: 'none', border: '2px solid var(--red)', borderRadius: '8px', padding: '10px 24px' }}>Call (610) 533-7791</a>
              </div>
            ) : (
              <>
                      ...
                      <div key={deal.id} style={{ background: 'var(--bg-2)', borderRadius: '10px', padding: '20px', border: '1px solid ' + (deal.active ? 'var(--green)' : 'var(--border-2)'), display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                            ...
                            <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: deal.admin_disabled ? 'var(--red)' : deal.active ? 'var(--green-dk)' : 'var(--ink-4)' }}>{deal.admin_disabled ? 'Disabled by admin' : deal.active ? 'Live' : 'Paused'}</span>
                          </div>
                          {deal.schedule ? (
                            <div style={{ fontSize: '11px', color: 'var(--ink-4)', fontWeight: 500 }}>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].filter((_,i) => deal.schedule!.days.includes(i)).join(', ')} {deal.schedule.start}-{deal.schedule.end}</div>
                          ) : <div style={{ fontSize: '11px', color: 'var(--ink-4)', fontWeight: 500 }}>Always available</div>}
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flexShrink: 0 }}>
                          {deal.admin_disabled ? (
                            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--red)', textAlign: 'center', padding: '4px 8px' }}>Contact PerkPass</div>
                          ) : (
                            <>
                              <button onClick={() => toggleDeal(deal)} disabled={togglingDeal === deal.id} className={deal.active ? 'btn btn-outline' : 'btn btn-primary'} style={{ fontSize: '14px', padding: '10px 20px', minWidth: '90px' }}>{togglingDeal === deal.id ? '...' : deal.active ? 'Pause' : 'Activate'}</button>
                              <button onClick={() => openScheduleEditor(deal)} style={{ fontSize: '12px', fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--green-dk)', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>{deal.schedule ? 'Edit hours' : 'Set hours'}</button>
                              <button onClick={() => setConfirmDelete(deal)} style={{ fontSize: '12px', fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--red)', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>Delete</button>
                            </>
                          )}
                        </div>
                      </div>
                ...
                <div style={{ marginTop: '24px', textAlign: 'center' }}>
                  <button onClick={() => setTab('submit')} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '14px', textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--green-dk)', background: 'none', border: 'none', cursor: 'pointer' }}>+ Submit a new deal</button>
                </div>
              </>
            )}
          </div>
        ...
        {tab === 'submit' && (
          <div>
            ...
            <p style={{ fontSize: '14px', fontWeight: 500, color: 'var(--ink-3)', marginBottom: '28px' }}>Propose a new deal for PerkPass members. We will review and activate it within 1-2 business days.</p>
            {submitSuccess ? (
      ...
      {editingSchedule && (() => {
        ...
        function fmt(t: string) { const [hh, mm] = t.split(':').map(Number); const ampm = hh >= 12 ? 'PM' : 'AM'; return (hh % 12 || 12) + ':' + String(mm).padStart(2,'0') + ' ' + ampm }
        return (
            ...
            <div style={{ background: 'var(--bg)', borderRadius: '16px', padding: '28px 24px', width: '100%', maxWidth: '420px', border: '2px solid var(--ink)' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <select value={schedStart} onChange={e => setSchedStart(e.target.value)} className="pp-input" style={{ flex: 1, padding: '10px 12px', fontSize: '15px', fontWeight: 600 }}>{TIMES.map(t => <option key={t} value={t}>{fmt(t)}</option>)}</select>
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, color: 'var(--ink-4)' }}>to</span>
                  <select value={schedEnd} onChange={e => setSchedEnd(e.target.value)} className="pp-input" style={{ flex: 1, padding: '10px 12px', fontSize: '15px', fontWeight: 600 }}>{TIMES.map(t => <option key={t} value={t}>{fmt(t)}</option>)}</select>
                </div>
              </div>
              <div style={{ background: 'var(--bg-2)', borderRadius: '8px', padding: '12px 14px', marginBottom: '20px', border: '1px solid var(--border)' }}>
                ...
                <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)' }}>{schedDays.length === 0 ? 'No days selected' : schedDays.length === 7 ? 'Every day, ' + fmt(schedStart) + '-' + fmt(schedEnd) : ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].filter((_,i) => schedDays.includes(i)).join(', ') + ', ' + fmt(schedStart) + '-' + fmt(schedEnd)}</div>
              </div>
              <button onClick={saveSchedule} disabled={savingSchedule || schedDays.length === 0} className="btn btn-primary" style={{ width: '100%', fontSize: '16px', padding: '14px', marginBottom: '8px' }}>{savingSchedule ? 'Saving...' : 'Save schedule'}</button>
              {editingSchedule.schedule && <button onClick={clearSchedule} disabled={savingSchedule} style={{ width: '100%', marginBottom: '8px', padding: '12px', background: 'none', border: '1px solid var(--border)', borderRadius: '8px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '14px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--ink-4)', cursor: 'pointer' }}>Remove schedule (always available)</button>}
              <button onClick={() => setEditingSchedule(null)} style={{ width: '100%', padding: '12px', background: 'none', border: 'none', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '14px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--ink-4)', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        )
      })()}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: 'var(--bg)', borderRadius: '16px', padding: '28px 24px', width: '100%', maxWidth: '380px', border: '2px solid var(--ink)', textAlign: 'center' }}>
            <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: 'var(--red-lt)', margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--red)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
            </div>
            ...
            <p style={{ fontSize: '13px', fontWeight: 500, color: 'var(--ink-3)', lineHeight: 1.5, marginBottom: '8px' }}>This action cannot be undone. The deal will be permanently removed and members will not see it anymore.</p>
            <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--ink-4)', marginBottom: '24px' }}>To offer a new deal, use the Submit Deal tab.</p>
            <button onClick={() => deleteDeal(confirmDelete)} disabled={deletingDeal === confirmDelete.id} style={{ width: '100%', marginBottom: '10px', padding: '14px', background: 'var(--red-lt)', border: '1px solid var(--red)', borderRadius: '8px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '16px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--red)', cursor: 'pointer' }}>{deletingDeal === confirmDelete.id ? 'Deleting...' : 'Yes, delete permanently'}</button>
            <button onClick={() => setConfirmDelete(null)} className="btn btn-primary" style={{ width: '100%', fontSize: '16px', padding: '14px' }}>Keep this deal</button>
          </div>
        </div>
      )}
    </main>
